...
public class TouchesTopoController extends StandaloneEventProcessingDeclarer<TouchesTopoParameter> {
    ...
    public final static String EPA_NAME = "Touches Topology Filter";


    @Override
    public DataProcessorDescription declareModel() {
        return ProcessingElementBuilder
                .create("org.streampipes.processors.geo.jvm.processors.topologicalOperators.filter.typeFilter.touches",
                        EPA_NAME,
                        "Topology filter with different touches types")
                ...
                .requiredSingleValueSelection(
                        ...
                        Options.from(
                                TouchesTypes.normal.name(),
                                TouchesTypes.interior.name(),
                                TouchesTypes.boundary.name()
                        )
                )
    ...
    @Override
    public ConfiguredEventProcessor<TouchesTopoParameter> onInvocation(DataProcessorInvocation graph, ProcessingElementParameterExtractor extractor){
        ...
        int type = 1;
        if (read_type.equals(TouchesTypes.interior.name())){
            type = TouchesTypes.interior.getNumber();
        } else if (read_type.equals(TouchesTypes.boundary.name())){
            type = TouchesTypes.boundary.getNumber();
        }


        TouchesTopoParameter params = new TouchesTopoParameter(graph, geom1, epsg_geom1, geom2, epsg_geom2, type, vsChecker);

        return new ConfiguredEventProcessor<>(params, TouchesTopo::new);
    }
}
